        ...
        <div class="col-12">
            <div class="panel">
                <div class="panel-body">
                    <form class="form-row post-form-edit" onsubmit="return false" data-parsley-validate>
                        <div class="col-md-6">
                            <div class="form-group parsley-form-group">
                                <label class="col-form-label" for="postTitleInput">文章标题</label>
                                <input class="form-control" id="postTitleInput" type="text" name="title" value="{{ $post->title }}"
                                ...
                                <div class="btn-group-toggle" data-toggle="buttons">
                                    @if(count($tags))
                                        @foreach($tags as $tag)
                                            ...
                                            {{ array_key_exists($tag->id, $tagArr) ? 'active' : '' }}">
                                                <input type="checkbox" name="tag_id[]" value="{{ $tag->id }}"
                                                       {{ array_key_exists($tag->id, $tagArr) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-9">
                            <div class="form-group parsley-form-group">
                                ...
                                <textarea class="form-control summernote" id="postContentInput" name="content" rows="10"
                                          data-parsley-required="true" data-parsley-required-message="文章内容为必填项"
                                          data-parsley-trigger="blur">{{ $post->content }}</textarea>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="panel-footer">
                                <button class="btn btn-primary js-btn-submit" type="submit"  data-m-id="{{ $post->id }}">保存修改</button>
                            </div>
                        </div>
                    </form>
                </div>
